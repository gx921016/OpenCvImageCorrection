...
-(void)setupCaptureManager:(CameraType)camera {
    ...
    if (_captureManager) {
        ...
        //Preview Layer setup
        CGRect layerRect = self.layer.bounds;//CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT-64-106);//self.layer.bounds;
        ...
        [videoView.layer addSublayer:_captureManager.previewLayer];
        //Add to self.view's layer
        WS(weakSelf);
        [videoView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.edges.equalTo(weakSelf).insets(UIEdgeInsetsMake(0, 0, 106, 0));
        }];
...
//        _cameraShutter.center = CGPointMake(self.center.x, self.center.y*1);
     ...
     [self.bottomView addSubview:_cameraShutter];
    [_cameraShutter mas_makeConstraints:^(MASConstraintMaker *make) {
